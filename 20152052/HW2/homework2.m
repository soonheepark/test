...
function [ground_anl, ground_num, E_err]=homework2(N)
%% input parameter
% N=5; % point # 
a=5.*1e-9; % scale [m]
d=a./(N-1); % 
...
% B=V/D
% eig_vec=find(min(-D(D~=0));
% E=ground.*h_bar.^2.*6.24e18./(2.*m_eff*(d.*1e-9)^2); %[J]
...
% norm(V,2)
% for i=1:N-2
%     norm_coeff(:,i)=norm(V(:,i),2);
%     y(:,i)=norm_coeff(:,i).*V(:,i);
% end

% plot(x,V2(:,1),x,V3(:,1))
% plot(x,V_Num_prob(:,3),x,V_Anal_prob(:,3))
% E1=E.*V;
